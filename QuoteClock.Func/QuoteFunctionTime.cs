...
namespace QuoteClock.Func
{
    ...
    {
        [FunctionName($"{nameof(QuoteFunctionTime)}_{nameof(GetRandom)}")]
        public async Task<IActionResult> GetRandom(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = ROUTE_TIME_RANDOM)] HttpRequest req,
            ILogger log)
        {
            log.LogInformation($"Getting Random Time");
        ...
        public async Task<IActionResult> GetCurrent(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = ROUTE_TIME_NOW)] HttpRequest req,
            ILogger log)
        {
